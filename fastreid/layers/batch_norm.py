...
class BatchNorm(nn.BatchNorm2d):
    def __init__(self, num_features, eps=1e-05, momentum=0.1, weight_freeze=False, bias_freeze=False, affine=True, weight_init=1.0,
...
class My_BatchNorm2d(nn.Module):

    def __init__(self, num_features, eps=1e-5, momentum=0.1, weight_freeze=False, affine=True, weight_init=1.0,
                 bias_init=0.0, **kwargs):
        super(My_BatchNorm2d, self).__init__()
        self.eps = eps
        ...
        if self.track_running_stats:
            ...
            self.register_buffer('running_mean_v', torch.zeros(num_features))
            self.register_buffer('running_var_v', torch.ones(num_features))
    ...
    def forward(self, input):
        ...
        if self.training:
            ...
            mean_i = input[:input.size(0) // 2].mean([0, 2, 3])
            mean_v = input[input.size(0) // 2:].mean([0, 2, 3])
            ...
            with torch.no_grad():
                ...
                self.running_mean_v = exponential_average_factor * mean_v \
                                      + (1 - exponential_average_factor) * self.running_mean_v
