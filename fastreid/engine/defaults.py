...
from fastreid.modeling.self_module import CLUB
...
class DefaultTrainer(TrainerBase):
    ...
    def __init__(self, cfg):
        ...
        data_loader,all_img_cams,gt_id_all_img = self.build_train_loader(cfg)
        ...
        # prasing network
        self.parsing_net = resnet101(num_classes=7, pretrained=None)
        ...
        for k, v in state_dict.items():
            ...
            if 'fushion' in name:
                fine_tune_state_dict[name] = v
        self.parsing_net.load_state_dict(new_state_dict)
        self.parsing_net.cuda()
        self.parsing_net.eval()
        self.fine_tune_net = FineTuneBlock(num_classes=7)
        self.fine_tune_net.load_state_dict(fine_tune_state_dict)
        self.fine_tune_net.cuda()
        self.fine_tune_net.train()

        self.optimizer_parsing = torch.optim.Adam(params=self.fine_tune_net.parameters(),lr=0.00001, weight_decay=cfg.SOLVER.WEIGHT_DECAY)
        optimizer_ckpt = dict(optimizer=optimizer)
        optimizer_parsing_ckpt = dict(optimizer=self.optimizer_parsing)

        # if cfg.SOLVER.FP16_ENABLED:
        #     model, optimizer = amp.initialize(model, optimizer, opt_level="O1")
        #     optimizer_ckpt.update(dict(amp=amp))

        #     self.fine_tune_net, self.optimizer_parsing = amp.initialize(self.fine_tune_net, self.optimizer_parsing, opt_level="O1")
        #     optimizer_parsing_ckpt.update(dict(amp=amp))

        self._trainer = (AMPTrainer if cfg.SOLVER.FP16_ENABLED else SimpleTrainer)(
            model, data_loader, optimizer, param_wrapper, self.optimizer_parsing, cfg, self.parsing_net, self.fine_tune_net)
        ...
        self.scheduler_parsing = self.build_lr_scheduler(cfg, self.optimizer_parsing, self.iters_per_epoch)
    ...
    def build_hooks(self):
        ...
        ret = [
            ...
            hooks.LRScheduler(self.optimizer_parsing, self.scheduler_parsing)
        ]
        ...
        def test_and_save_results():
            self._last_eval_results = self.test(self.cfg, self.model, self.fine_tune_net, self.parsing_net)
            return self._last_eval_results

        # Do evaluation before checkpointer, because then if it fails,
        # we can use the saved checkpoint to debug.
        ret.append(hooks.EvalHook(cfg.TEST.EVAL_PERIOD, test_and_save_results))
        ret.append(hooks.Visu_Hook(self.model, cfg.OUTPUT_DIR))
        ret.append(hooks.Parsing_hook(self.parsing_net, self.fine_tune_net))

        if comm.is_main_process():
            ret.append(hooks.PeriodicCheckpointer(self.checkpointer, cfg.SOLVER.CHECKPOINT_PERIOD))
            ret.append(hooks.Save_Checkpoint(self.fine_tune_net, cfg.SOLVER.CHECKPOINT_PERIOD, cfg))
    ...
    @classmethod
    def test(cls, cfg, model, fine_tune_org, parsing_net):
        """
        Args:
            cfg (CfgNode):
            model (nn.Module):
        ...
        for idx, dataset_name in enumerate(cfg.DATASETS.TESTS):
            ...
            try:
                ...
                continue
            results_i = inference_on_dataset(model, fine_tune_org, parsing_net, data_loader, evaluator, flip_test=cfg.TEST.FLIP_ENABLED)
    ...
    def auto_scale_hyperparams(cfg, num_classes):
        ...
        if cfg.MODEL.HEADS.NUM_CLASSES == []:
            output_dir = cfg.OUTPUT_DIR
