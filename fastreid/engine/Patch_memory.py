...
class PatchMemory(nn.Module):
    ...
    def __init__(self, num_feature_dims, num_samples, model, train_loader, temp=0.05, momentum=0.2):
        super(PatchMemory, self).__init__()
        ...
        self.register_buffer('features', torch.zeros((self.num_samples*6, self.num_features)))
    ...
    def initial_memory(self):
        ...
        with torch.no_grad():
            for i, data in enumerate(self.train_loader):
                if i <= 300:
                    feats, patch_features = self.model(data)  #patch_features :[bs, num_patch, feature_dim]
                    patch_features_trans = patch_features.transpose(0,1)#[num_patch, bs, feature_dim]
                    fnames = data['img_paths']
                    pids = data['targets']
                    camids = data['camids']
                    indexs = data['index']
                    for patch_id, patch_feature_trans in enumerate(patch_features_trans):
                        if patch_id == 0:
                            for fname, feat, pid, camid,index in zip(fnames, patch_feature_trans, pids, camids,indexs):
                                self.labels[int(index)] = pid.cuda()
                                self.camids[int(index)] = camid
                                self.features[int(index)] = feat
                        else:
                            for fname, feat, pid, camid, index in zip(fnames, patch_feature_trans, pids, camids,
                                                                      indexs):
                                self.features[int(index)*(patch_id+1)] = feat
                else:break
        self.model.train()
        print(self.labels)
        print('initial finish. Size of patch feature is {}'.format(self.features.size()))
